...
public class TapSelDetailManager : MonoBehaviour
{
    public GameObject tapSelItem;
    public GameObject tapSelParent;
    public GameObject selDayPopup;
    public Text selStartDayTxt;
    public Text selEndDayTxt;
    ...
    List<GameObject> m_tapselObj = new List<GameObject>();
    int sYear;
    int sMonth;
    int sDay;
    int eYear;
    ...
    int is_sel_type = 0;//1-시작날짜 선택, 2-마감날짜 선택
    ...
    void Start()
    {
        ...
        selected_start_date = Global.GetSdate();
        selected_end_date = selected_start_date;
        titleTxt.text = "TAP" + Global.cur_tap_no + " 상세내역";
        sYear = selected_start_date.Year;
        sMonth = selected_start_date.Month;
        sDay = selected_start_date.Day;
        eYear = selected_end_date.Year;
        eMonth = selected_end_date.Month;
        eDay = selected_end_date.Day;
        int _sy = 0;
        try
        {
            _sy = int.Parse(sYear.ToString().Substring(2, 2));
        }
        catch (Exception ex)
        {
        ...
        try
        {
            _ey = int.Parse(eYear.ToString().Substring(2, 2));
        }
        catch (Exception ex)
        {

        }
        selStartDayTxt.text = string.Format("{0:D2}.{1:D2}.{2:D2}", _sy, sMonth, sDay);
        selEndDayTxt.text = string.Format("{0:D2}.{1:D2}.{2:D2}", _ey, eMonth, eDay);
        SendRequest();
    ...
    {
        StartCoroutine(GotoScene("tapselDetail"));
    }
    ...
    void onSelectDay(int day)
    {
        if (is_sel_type == 1)
        {
            selected_start_date = new DateTime(sYear, sMonth, day);
            int _y = 0;
            try
            {
                _y = int.Parse(selected_start_date.Year.ToString().Substring(2, 2));
            }
            catch (Exception ex)
            {

            }
            selStartDayTxt.text = string.Format("{0:D2}.{1:D2}.{2:D2}", _y, selected_start_date.Month, selected_start_date.Day);
        }
        else
        {
            selected_end_date = new DateTime(eYear, eMonth, day);
            int _y = 0;
            try
            {
                _y = int.Parse(selected_end_date.Year.ToString().Substring(2, 2));
            }
            catch (Exception ex)
            {

            }
            selEndDayTxt.text = string.Format("{0:D2}.{1:D2}.{2:D2}", _y, selected_end_date.Month, selected_end_date.Day);
        }
    ...
    void SendRequest()
    {
        WWWForm form = new WWWForm();
        form.AddField("pub_id", Global.userinfo.pub.id);
        form.AddField("tap_id", Global.cur_tap_id);
        form.AddField("startdate", string.Format("{0:D4}-{1:D2}-{2:D2}", selected_start_date.Year, selected_start_date.Month, selected_start_date.Day));
        form.AddField("enddate", string.Format("{0:D4}-{1:D2}-{2:D2}", selected_end_date.Year, selected_end_date.Month, selected_end_date.Day));
        WWW www = new WWW(Global.api_url + Global.get_tapsel_detail_api, form);
    ...
    IEnumerator LoadInfo(WWW www)
    {
        ...
        if (www.error == null)
        {
            List<TapSellInfo> tslist = new List<TapSellInfo>();
            JSONNode jsonNode = SimpleJSON.JSON.Parse(www.text);
            JSONNode tlist = JSON.Parse(jsonNode["taplist"].ToString()/*.Replace("\"", "")*/);
            for (int i = 0; i < tlist.Count; i++)
            {
                TapSellInfo tinfo = new TapSellInfo();
                tinfo.no = tlist[i]["no"].AsInt;
                tinfo.name = tlist[i]["name"];
                tinfo.start_date = tlist[i]["start_date"];
                tinfo.sell_period = tlist[i]["sell_period"];
                tinfo.normal_sell_cnt = tlist[i]["normal_sell_cnt"];
                tinfo.ml_sell_cnt = tlist[i]["ml_sell_cnt"].AsInt;
                tinfo.cancel_sell_cnt = tlist[i]["cancel_sell_cnt"].AsInt;
                tinfo.out_capacity = tlist[i]["out_capacity"].AsInt;
                tinfo.cancel_capacity = tlist[i]["cancel_capacity"].AsInt;
                tinfo.sell_capacity = tlist[i]["sell_capacity"].AsInt;
                tinfo.sell_price = tlist[i]["sell_price"].AsInt;
                tslist.Add(tinfo);
            }
            //UI에 추가
            for (int i = 0; i < tslist.Count; i++)
            {
                ...
                try
                {
                    ...
                    tmp.transform.Find("name").GetComponent<Text>().text = tslist[i].name;
                    tmp.transform.Find("startday").GetComponent<Text>().text = tslist[i].start_date;
                    tmp.transform.Find("period").GetComponent<Text>().text = tslist[i].sell_period;
                    tmp.transform.Find("sell").GetComponent<Text>().text = tslist[i].normal_sell_cnt + " 회";
                    tmp.transform.Find("ml").GetComponent<Text>().text = tslist[i].ml_sell_cnt + " 회";
                    tmp.transform.Find("cancel").GetComponent<Text>().text = tslist[i].cancel_sell_cnt + " 회";
                    tmp.transform.Find("size").GetComponent<Text>().text = tslist[i].out_capacity + " ml";
                    tmp.transform.Find("cancel_size").GetComponent<Text>().text = tslist[i].cancel_capacity + " ml";
                    tmp.transform.Find("sell_size").GetComponent<Text>().text = tslist[i].sell_capacity + " ml";
                    tmp.transform.Find("sell_price").GetComponent<Text>().text = Global.GetNoFormat(tslist[i].sell_price);
                }
                catch (Exception ex)
                {
    ...
    public void onSelStartDay()
    {
        ...
        else
        {
            ...
            selMonthTxt.text = string.Format("{0:D4}.{1:D2}", sYear, sMonth);
            LoadDays();
        }
    }

    public void onSelEndDay()
    {
        if (selDayPopup.activeSelf)
        {
            is_sel_type = 0;
        ...
        else
        {
            is_sel_type = 2;
            selDayPopup.SetActive(true);
            selMonthTxt.text = string.Format("{0:D4}.{1:D2}", eYear, eMonth);
            LoadDays();
        }
    }
    ...
    void LoadDays()
    {
        ...
        if (is_sel_type == 1)
        {
            cur_date = new DateTime(sYear, sMonth, 1);
        }
        else
        {
            cur_date = new DateTime(eYear, eMonth, 1);
        }
        int freedays = 0;
        ...
        for (int i = 0; i < 42; i++)
        {
            ...
            else if (i < freedays + daysCnt)
            {
                ...
                if (is_sel_type == 1)
                {
                    if (sYear == selected_start_date.Year && sMonth == selected_start_date.Month && selected_start_date.Day == (i - freedays + 1))
                    {
                ...
                {
                    if (eYear == selected_end_date.Year && eMonth == selected_end_date.Month && selected_end_date.Day == (i - freedays + 1))
                    {
                        selDayPopup.transform.Find("background/days").gameObject.transform.GetChild(i).GetComponent<Image>().sprite = Resources.Load<Sprite>("seldate");
                    }
                    else
    ...
    public void onNextMonth()
    {
        if (is_sel_type == 1)
        {
            DateTime curDate = new DateTime(sYear, sMonth, 1).AddMonths(1);
            sYear = curDate.Year;
            sMonth = curDate.Month;
            selMonthTxt.text = string.Format("{0:D4}.{1:D2}", sYear, sMonth);
        }
        else
        {
            DateTime curDate = new DateTime(eYear, eMonth, 1).AddMonths(1);
            eYear = curDate.Year;
            eMonth = curDate.Month;
            selMonthTxt.text = string.Format("{0:D4}.{1:D2}", eYear, eMonth);
        }
        LoadDays();
    }
