...
public class TableMenuOrderManager : MonoBehaviour
{
    ...
    int is_all_pay = 0;//0-주문후결제, 1-전체결제
    ...
    IEnumerator LoadOrderList()
    {
        ...
        form.AddField("pub_id", Global.userinfo.pub.id);
        WWW www = new WWW(Global.api_url + Global.get_tableorderlist_api, form);
    ...
    IEnumerator GetTableorderlistFromApi(WWW www)
    {
        ...
        if (www.error == null)
        {
            ...
            tagPriceTxt.text = Global.GetPriceFormat(tableorderlist.tag_price);
            ...
            Global.tableUsageInfo.tableId = Global.cur_tInfo.tid;
    ...
    void payProcess()
    {
        if (popup_type == 6)
        {
            bool isPriceFormat = false;
            string priceStr = payPopup.transform.Find("background/1/payamount").GetComponent<InputField>().text;
            ...
            try
            {
                if (isPriceFormat)
                {
                    pay_price = Global.GetConvertedPrice(priceStr) - used_prepay;
                }
                else
                {
                    pay_price = int.Parse(priceStr) - used_prepay;
                }
            } catch(Exception ex)
            {
                err_popup.SetActive(true);
                err_str.text = "결제금액을 옳바로 입력하세요.";
                if (Global.setinfo.pos_no == 1)
                {
                    socket1.Emit("endpay");
                }
                        ...
                        for (int i = 0; i < orderlist.Count; i++)
                        {
                            ...
                            for (int j = 0; j < printList.Count; j++)
                            {
                                if (printList[j].product_name == orderlist[i]["product_name"] && printList[j].is_service == orderlist[i]["is_service"].AsInt)
                                {
                                    ...
                                    temp.quantity += orderlist[i]["quantity"].AsInt;
            ...
            try
            {
                if (m_orderListObj[i].transform.Find("check").GetComponent<Toggle>().isOn)
                {
                    if(m_orderListObj[i].transform.Find("order_ids").GetComponent<Text>().text.Trim() == "")
                    {
                        ...
                        oinfo += "\"menu_id\":\"" + m_orderListObj[i].transform.Find("menu_id").GetComponent<Text>().text + "\","
        ...
        if (www.error == null)
        {
            try
            {
                ...
                if (jsonNode["suc"].AsInt == 1)
                {
                    ...
                    tmp.transform.Find("menu_id").GetComponent<Text>().text = "0";
                    tmp.transform.Find("type").GetComponent<Text>().text = "0";
                    tmp.transform.Find("status").GetComponent<Text>().text = "0";
                    tmp.transform.Find("name").GetComponent<Text>().color = Global.selected_color;
                    tmp.transform.Find("cnt").GetComponent<Text>().color = Global.selected_color;
                    tmp.transform.Find("price").GetComponent<Text>().color = Global.selected_color;
                    tmp.transform.Find("product_type").GetComponent<Text>().text = "0";
        ...
        for (int i = 0; i < m_orderListObj.Count; i++)
        {
            try
            {
                if (m_orderListObj[i].transform.Find("check").GetComponent<Toggle>().isOn)
                {
                    ...
                    if (m_orderListObj[i].transform.Find("order_ids").GetComponent<Text>().text.Trim() == "")
                    {
                        ...
                        for (int j = 0; j < cartlist.Count; j++)
                        {
                            if (cartlist[j].menu_id == item)
                            {
                                ...
                                total_priceTxt.text = (total_price).ToString();
                    ...
                    {
                        if (m_orderListObj[i].transform.Find("status").GetComponent<Text>().text == "2" || m_orderListObj[i].transform.Find("status").GetComponent<Text>().text == "3")
                        {
                            is_cooking = true;
                        }
