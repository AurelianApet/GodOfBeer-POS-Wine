...
public class RepayManager : MonoBehaviour
{
    ...
    public Toggle selAllTags;
    public Toggle selAllMenu;
    public Toggle selAllUsage;

    public GameObject select_popup;
    public Text select_popup_str;
    ...
    void Start()
    {
        ...
        WWW www = new WWW(Global.api_url + Global.get_repay_api, form);
    ...
    IEnumerator LoadInfo(WWW www)
    {
        ...
        if (www.error == null)
        {
            ...
            for (int i = 0; i < tableTag_list.Count; i++)
            {
                ...
                for (int j = 0; j < tagorder_list.Count; i++)
                {
                    ...
                    ttInfo.status = tagorder_list[i]["status"].AsInt;
                    ...
                    //tmp.transform.GetComponent<RectTransform>().sizeDelta = new Vector2(-(left + right), 0);
                    //tmp.transform.localScale = Vector3.one;
                    tmpObj.transform.Find("tag").GetComponent<Text>().text = tgInfo.tagName;
                    tmpObj.transform.Find("menu").GetComponent<Text>().text = ttInfo.menu_name;
                    tmpObj.transform.Find("time").GetComponent<Text>().text = ttInfo.reg_datetime;
                    tmpObj.transform.Find("cnt").GetComponent<Text>().text = ttInfo.amount.ToString();
                    tmpObj.transform.Find("price").GetComponent<Text>().text = ttInfo.price.ToString();
                    tmpObj.transform.Find("tag_id").GetComponent<Text>().text = tgInfo.tag_id.ToString();
                    tmpObj.transform.Find("order_id").GetComponent<Text>().text = ttInfo.order_id.ToString();
                    tmpObj.transform.Find("status").GetComponent<Text>().text = ttInfo.status.ToString();
                    GameObject tgObj = tmpObj.transform.Find("check").gameObject;
                    tmpObj.GetComponent<Button>().onClick.RemoveAllListeners();
                    tmpObj.GetComponent<Button>().onClick.AddListener(delegate () { onClickOrderItem(tgObj, ttInfo.order_id, 0); });
                    using_price += ttInfo.price;
                    mtableUsageObj.Add(tmpObj);
                    selAllUsage.onValueChanged.RemoveAllListeners();
                    selAllUsage.onValueChanged.AddListener((value) => {
                        onSelectAllUsage(value);
                    }
                    );
                }
            ...
            selAllTags.onValueChanged.RemoveAllListeners();
            selAllTags.onValueChanged.AddListener((value) => {
                onSelectAllTags(value);
            }
            );
    ...
    void onSelectAllTags(bool value)
    {
        for (int i = 0; i < tagItemParent.transform.childCount; i++)
        {
            tagItemParent.transform.GetChild(i).Find("check").GetComponent<Toggle>().isOn = value;
        }
        string payInfo = getJsonResult();
        Debug.Log(payInfo);
        if (Global.setinfo.pos_no == 1)
    ...
    void onSelectAllUsage(bool value)
    {
        for (int i = 0; i < tableUsageItemParent.transform.childCount; i++)
        {
            tableUsageItemParent.transform.GetChild(i).Find("check").GetComponent<Toggle>().isOn = value;
        }
        string payInfo = getJsonResult();
        Debug.Log(payInfo);
        if (Global.setinfo.pos_no == 1)
    ...
    void onSelectAllMenu(bool value)
    {
        for (int i = 0; i < menuItemParent.transform.childCount; i++)
        {
            try
            {
                ...
                menuItemParent.transform.GetChild(i).Find("check").GetComponent<Toggle>().isOn = value;
            }
            catch (Exception ex)
            {
                Debug.Log(ex);
        ...
        if (toggleObj.GetComponent<Toggle>().isOn)
        {
            ...
            if (type == 1)
            {
                ...
                for (int i = 0; i < mtableUsageObj.Count; i++)
                {
                    ...
                    {
                        int p = Global.GetConvertedPrice(mtableUsageObj[i].transform.Find("price").GetComponent<Text>().text);
                        selected_price -= p;
                        selectedPriceTxt.text = Global.GetPriceFormat(selected_price);
                        using_price -= p;
                        usageSumPriceTxt.text = Global.GetPriceFormat(using_price);
        ...
        else
        {
            ...
            if (type == 1)
            {
                ...
                for (int j = 0; j < Global.tableUsageInfo.tagUsageList[tag_index].tagMenuOrderList.Count; j++)
                {
                    ...
                    tmp.transform.Find("price").GetComponent<Text>().text = Global.tableUsageInfo.tagUsageList[tag_index].tagMenuOrderList[j].price.ToString();
                    ...
                    tmp.GetComponent<Button>().onClick.RemoveAllListeners();
                    tmp.GetComponent<Button>().onClick.AddListener(delegate () { onClickOrderItem(tObj, Global.tableUsageInfo.tagUsageList[tag_index].tagMenuOrderList[j].order_id, 0); });
                    selected_price += Global.tableUsageInfo.tagUsageList[tag_index].tagMenuOrderList[j].price;
                    selectedPriceTxt.text = Global.GetPriceFormat(selected_price);
                    using_price += Global.tableUsageInfo.tagUsageList[tag_index].tagMenuOrderList[j].price;
                    usageSumPriceTxt.text = Global.GetPriceFormat(using_price);
    ...
    int pay_type = 0;//0-all pay, 1-selected pay
    ...
    public void SelectPay()
    {
        ...
        if (pay_price == 0)
        {
            ...
            if (is_found)
            {
                ...
                select_popup_str.text = "결제금액이 0원입니다. 결제처리하시겠습니까?";
    ...
    public void AllPay()
    {
        ...
        if (pay_price == 0)
        {
            ...
            if (is_found)
            {
                ...
                select_popup_str.text = "결제금액이 0원입니다. 결제처리하시겠습니까?";
                select_popup.transform.Find("background/confirmBtn").GetComponent<Button>().onClick.RemoveAllListeners();
                select_popup.transform.Find("background/confirmBtn").GetComponent<Button>().onClick.AddListener(delegate () { onZeroPay(1); });
                return;
            }
            err_popup.SetActive(true);
            err_str.text = "결제금액이 없습니다.";
            ...
            if (jsonNode["suc"].AsInt == 1)
            {
                for (int j = 0; j < tags.Count; j++)
                {
                    for (int i = 0; i < Global.tableUsageInfo.tagUsageList.Count; i++)
                    {
                        ...
                        {
                            Global.tableUsageInfo.tagUsageList.RemoveAt(i); break;
                        }
                    }
                }
                ...
                for (int j = 0; j < tags.Count; j++)
                {
                    for (int i = 0; i < mtagObj.Count; i++)
                    {
                        try
                        {
                            if (mtagObj[i].transform.Find("tag_id").GetComponent<Text>().text == tags[j].ToString())
                            {
                                DestroyImmediate(mtagObj[i].gameObject);
                                mtagObj.Remove(mtagObj[i]);
        ...
        for (int i = 0; i < mMenuObj.Count; i++)
        {
            try
            {
                if (mMenuObj[i].transform.Find("check").GetComponent<Toggle>().isOn)
                {
                    ...
                    total_price -= Global.GetConvertedPrice(mMenuObj[i].transform.Find("price").GetComponent<Text>().text);
                    if (mMenuObj[i].transform.Find("status").GetComponent<Text>().text == "0")//state : 0이면 결제취소된 주문메뉴
                    {
                        DestroyImmediate(mMenuObj[i]);
                        mMenuObj.Remove(mMenuObj[i]);
                        i--;
                    }
                    else//기타 재결제화면에서 결제한 선결제내역
                    {
    ...
    public void onConfirmPopup()
    {
        ...
        DateTime dt = Global.GetSdate();
        form.AddField("sdate", string.Format("{0:D4}-{1:D2}-{2:D2}", dt.Year, dt.Month, dt.Day));
        WWW www = new WWW(Global.api_url + Global.cancel_prepay_api, form);
    ...
    public void onService()
    {
        ...
        {
            select_popup.SetActive(true);
            select_popup_str.text = "선택한 메뉴를 서비스 처리/취소 하시겠습니까?";
            select_popup.transform.Find("background/confirmBtn").GetComponent<Button>().onClick.RemoveAllListeners();
